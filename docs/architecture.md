...
`wowdata-server mcp http` is the remote service runtime. It loads the HTTP YAML
config, binds `/mcp`, `/health`, `/help`, and `/files/`, and serves MCP
Streamable HTTP requests. HTTP uses lazy prepare instead of exposing
`wow_warmup` by default:
