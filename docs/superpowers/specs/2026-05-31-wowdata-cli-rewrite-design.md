...
**架构：** 用一套共享的 Go 核心实现 CASC、DB2/WDC/DBC、BLTE、BLP、listfile、item、creature、decor 和诊断逻辑；CLI 层只负责子命令路由，并默认输出确定性的 JSON。当前 Node 仓库是必须复刻的能力基线，直到 Go 版对所有已暴露和未暴露能力都通过黄金样本对照。
...
它必须完整复刻当前 Node 仓库里的全部业务能力，而不只是当前的 MCP 工具。
这里的“完整复刻”包括：

- 已通过 MCP 暴露的能力
- core 模块中存在但尚未通过 MCP 暴露的能力
- 现有缓存、导出、诊断和查询语义
- 现有本地/远程 source 行为
- 现有错误路径中对业务有意义的行为
...
输出格式可以比 Node 更适合 CLI，但输出内容必须覆盖 Node 的业务信息。对现有 MCP 调用方可能依赖的字段，Go CLI 要么保留等价字段，要么在兼容说明中记录映射关系。
...
- 未暴露的 Node core 能力也必须被清点、迁移、测试；不能因为旧版没有 MCP 工具就跳过。
