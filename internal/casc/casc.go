...
func (c *CASCSource) parseRoot(data []byte) (int, error) {
	...
	if magic == rootMagic {
		...
		if version == 0 {
			...
			totalFileCount = binary.LittleEndian.Uint32(data[pos:]); pos += 4
			namedFileCount = binary.LittleEndian.Uint32(data[pos:]); pos += 4
		}
...
func (c *CASCSource) parseRootBlocks(data []byte, pos *int, version uint32, allowNameless, classic bool) (int, error) {
	for *pos < len(data) {
		...
		numRecords := binary.LittleEndian.Uint32(data[*pos:]); *pos += 4

		var contentFlags ContentFlag
		var localeFlags LocaleFlag

		if classic || version <= 1 {
			contentFlags = ContentFlag(binary.LittleEndian.Uint32(data[*pos:])); *pos += 4
			localeFlags = LocaleFlag(binary.LittleEndian.Uint32(data[*pos:])); *pos += 4
		} else if version == 2 {
			localeFlags = LocaleFlag(binary.LittleEndian.Uint32(data[*pos:])); *pos += 4
			cflags1 := binary.LittleEndian.Uint32(data[*pos:]); *pos += 4
			cflags2 := binary.LittleEndian.Uint32(data[*pos:]); *pos += 4
			cflags3 := uint32(data[*pos]); *pos++
		...
		for i := uint32(0); i < numRecords; i++ {
			nextID := fdid + uint32(int32(binary.LittleEndian.Uint32(data[*pos:]))); *pos += 4
			fdids[i] = nextID
			fdid = nextID + 1
		}
		...
		for i := uint32(0); i < numRecords; i++ {
			...
			key := hex.EncodeToString(data[*pos : *pos+16]); *pos += 16
			if classic {
				*pos += 8 // skip hash
			}
			entry[typeIndex] = key
...
func (c *CASCSource) parseEncoding(data []byte) error {
	...
	pos := 3 // version(1) + hashSizeCKey(1) + hashSizeEKey(1)

	hashSizeCKey := int(data[2])
	hashSizeEKey := int(data[3])
	cKeyPageSize := int(int16(binary.BigEndian.Uint16(data[4:]))) * 1024; pos = 6
	pos += 2 // eKeyPageSize
	cKeyPageCount := int(int32(binary.BigEndian.Uint32(data[pos:]))); pos += 4
	pos += 5 // eKeyPageCount + unk11
	specBlockSize := int(int32(binary.BigEndian.Uint32(data[pos:]))); pos += 4
	...
	for i := 0; i < cKeyPageCount; i++ {
		...
		for pos < pageStart+cKeyPageSize && pos < len(data) {
			keysCount := data[pos]; pos++
			if keysCount == 0 {
				break
			}

			// int40BE size
			size := int64(data[pos])<<32 | int64(binary.BigEndian.Uint32(data[pos+1:])); pos += 5

			cKey := hex.EncodeToString(data[pos : pos+hashSizeCKey]); pos += hashSizeCKey
			eKey := hex.EncodeToString(data[pos : pos+hashSizeEKey]); pos += hashSizeEKey
...
func (c *CASCSource) ParseArchiveIndex(data []byte, archiveKey string) {
	...
	for i := 0; i < count && pos+24 <= len(data); i++ {
		hash := hex.EncodeToString(data[pos : pos+16]); pos += 16
